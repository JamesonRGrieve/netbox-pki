...
is (optionally) terminated by a running :class:`netbox_services.ServiceInstance` — ``Certificate``
...
here. ``Certificate.key_ref`` / ``Certificate.cert_ref`` and ``ACMEAccount.account_key_ref`` /
``ACMEAccount.eab_hmac_ref`` and ``CertificateAuthority.ca_cert_ref`` are **OpenBao path
...
class NetBoxPKIConfig(PluginConfig):
    ...
    min_version = "4.6.0"
    max_version = "4.6.99"
    # Certificate.service_instance FKs netbox_services.ServiceInstance and the migration depends on
